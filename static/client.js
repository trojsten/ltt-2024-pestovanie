document.addEventListener("DOMContentLoaded", () => {
  ...
  plantBtn.addEventListener("click", async () => {
    ...
    plantList.forEach((plant) => {
      plant.addEventListener("click", async (e) => {
        if (confirm("Chceš naozaj zasadiť túto rastlinu?")) {
  ...
  breedBtn.addEventListener("click", async () => {
    ...
    animalList.forEach((animal) => {
      animal.addEventListener("click", async (e) => {
        if (confirm("Chceš naozaj chovať toto zviera?")) {
  ...
  craftBtn.addEventListener("click", async () => {
    if (gameData?.crafting != undefined) return;
    ...
    recipeList.forEach((recipe) => {
      recipe.addEventListener("click", async (e) => {
        if (confirm("Chceš naozaj vyrobiť tento predmet?")) {
  ...
  function isGrowing(growData) {
    ...
    if (growData.item.type == "plant") {
      if (!growData.watered)
        return growData.elapsed < (growData.item.time * TIME_MULTIPLIER) / 2;
  ...
  function isWaiting(data) {
    return (
      data.item.type !== "craft" &&
      data.elapsed >= (data.item.time * TIME_MULTIPLIER) / 2 &&
      (data.item.type == "plant" ? !data.watered : !data.fed)
    );
  }
  ...
  async function updateState(growData) {
    ...
    const page =
      growData[typeMap[growData.item.type]] | (growData.item.type == "craft")
        ? "claim"
        : "water";
    console.log(`/${growData.item.type}/${page}`);
    const res = await fetch(`/${growData.item.type}/${page}`, {
      method: "post",
    });
